...
CREATE TABLE `tbl_msg_template` (
  ...
  `bind_id` bigint(0) NULL DEFAULT NULL COMMENT '关联主键ID。如是有关联主键ID，则短信发送前需要检查前一条关联短信是否验证通过。',
  `template` varchar(16) NOT NULL COMMENT '短信模板。按照短信模板填充短信内容，通用替换字段：${verifycode},${year},${month},${day},${time},${datetime},${userId},${userName}',
  `auth_type` int(0) NOT NULL COMMENT '短信发送授权控制。0.不需要授权；1.需要授权；2.可授权可不授权',
  `auth_uri` int(0) NULL DEFAULT NULL COMMENT '短信授权URI。URI为空则是通用短信，不为空则是特定URI用途的短信。',
