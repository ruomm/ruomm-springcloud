...
CREATE TABLE `tbl_msg_template`  (
    ...
    `repeat_skip_time` int(0) NULL DEFAULT NULL COMMENT '再次发送间隔(单位秒)',
    `limit_by_term` int(0) NULL DEFAULT NULL COMMENT '同一设备，一天发送的次数限制',
    `limit_by_user` int(0) NULL DEFAULT NULL COMMENT '同一用户，一天发送的次数限制',
    `limit_by_addr` int(0) NULL DEFAULT NULL COMMENT '同一发送目标，一天发送的次数限制',
